...
for (let i = 0; i < TRIP_POINT_COUNT; i++) {
  render(siteMainElement.querySelector(`.trip-events__list`), tripPointTemplate(), `beforeend`);
}
const points = siteMainElement.querySelectorAll(`.trip-events__item`);
points[0].innerHTML = ``;
render(points[0], formEditTemplate(), `afterbegin`);
points[points.length - 1].innerHTML = ``;
render(points[points.length - 1], formCreateTemplate(), `afterbegin`);
